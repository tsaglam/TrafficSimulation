...
Junction& DomainModel::getJunction(id_type id) const {
  return *junctions.at(id);
}
const std::vector<std::unique_ptr<Vehicle>>& DomainModel::getVehicles() const {
  return vehicles;
}
const std::vector<std::unique_ptr<Street>>& DomainModel::getStreets() const {
  return streets;
}
const std::vector<std::unique_ptr<Junction>>& DomainModel::getJunctions()
    const {
  return junctions;
}
